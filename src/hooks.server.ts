...
import { runWithRequestContext, setActingUser } from '$lib/server/request-context';
...
export const handle: Handle = ({ event, resolve }) =>
	// The request context lets services (event-log attribution) read the
	// acting user without every route threading `locals.session` down.
	runWithRequestContext(async () => {
					...
					if (user) {
						...
						if (user.passwordChangedAt) {
							...
							if (passwordChangedTimestamp > tokenPasswordTimestamp) {
								...
								console.info(
									`[Security] Session revoked for user ${user.email} - password changed`,
								);
		...
		// Expose the authenticated user (from any of the three auth paths above)
		// to request-context consumers such as event-log attribution.
		if (event.locals.session?.user) {
			setActingUser(event.locals.session.user);
		}
