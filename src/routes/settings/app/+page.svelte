...
<div class="settings-section" id="storage" class:active={activeSection() === 'storage'}>
	...
	<div class="form-row">
		...
		<div class="form-group">
			...
			{:else if s.diskTotalGB}
				<p class="help-text">
					Default cache budget per user. {s.diskTotalGB.toFixed(1)} GB total on disk. Override per user
					in the Users tab.
				</p>
			{:else}
				<p class="help-text">
					Default cache budget per user. Oldest downloads are auto-removed when exceeded. Override
			...
			{:else}
				<p class="help-text">
					Global cap across all users. Blank = auto{s.autoTotalCacheGB !== null
						...
						: ' (disk − 5 GB)'}. Oldest cached items across all users are auto-removed when
					exceeded. Use this when not bounded by a PVC.
				</p>
			{/if}
		</div>
	</div>
	...
	<div class="form-group">
		...
		<p class="help-text">
			Removes stale partial downloads and leftover temporary files that no download owns, plus empty
			artwork-only library folders. Runs automatically every 5 minutes; this forces it immediately.
			In-flight downloads are never touched.
		</p>
	...
	{#if s.libraryEnabled}
		<div class="form-group nested-field">
			...
			<PathBrowser id="libraryPath" bind:value={s.settings.libraryPath} placeholder="/media" />
		</div>
		...
		<div class="form-group nested-field">
			...
			/>
			<p class="help-text">
				Audio-only downloads go here instead. Leave empty to use the video library path for
				everything.
			</p>
		</div>
	{/if}
	...
	<div class="form-row">
		...
		<div class="form-group">
			...
			<input
				type="number"
				id="maxDuration"
				value={s.settings.maxDurationSeconds ? Math.round(s.settings.maxDurationSeconds / 3600) : 3}
				oninput={(e) => {
	...
	<div class="form-row">
		<div class="form-group">
			...
			<input type="text" id="rateLimit" bind:value={s.settings.rateLimit} placeholder="Unlimited" />
	...
	<div class="form-group">
		...
		{#if s.aria2cSocksConflict}
			<p class="help-text error-text" role="alert">
				aria2c only supports HTTP proxies — it can't be combined with a SOCKS proxy URL, every
				download would fail. This toggle won't be saved while a socks5:// proxy is set.
			</p>
		{:else}
			<p class="help-text">
				Use aria2c accelerated downloader (requires aria2 in image). Note: aria2c only supports HTTP
				proxies, not SOCKS.
			</p>
		{/if}
	</div>
...
<div
	...
	id="library-access"
	class:active={activeSection() === 'library-access'}
>
	<h2>Access</h2>
	<div class="form-group">
		...
		<p class="help-text">
			Controls whether non-admin users can save downloads to the permanent library. Per-user
...
<div class="settings-section" id="ytdlp" class:active={activeSection() === 'ytdlp'}>
	...
	<div class="form-group">
		...
		></textarea>
		<p class="help-text">
			Server-wide default yt-dlp flags applied to every download and subscription check. Linked
			YouTube accounts can override these. One token per line — a flag and its value go on separate
			lines. Profile and per-subscription flags override these.
		</p>
	</div>
</div>
...
<div class="settings-section" id="ryd" class:active={activeSection() === 'ryd'}>
	...
	<div class="form-group">
		...
		<p class="help-text">
			Fetch dislike counts from the Return YouTube Dislike API when downloading videos. When
			enabled, video IDs are sent to an external service (<a
	...
	{#if s.jellyfinEnabled}
		<div class="form-row nested-field">
			...
			<div class="form-group">
				...
				/>
				<p class="help-text">
					Path mapping for the library setup: how the shared volume is mounted here vs in Jellyfin
					(e.g. /media → /media/youtube). Leave both empty when both containers use the same path.
				</p>
			</div>
			<div class="form-group">
		...
		<div class="jellyfin-library-setup nested-field">
			<div class="info-box">
				<strong>Library type:</strong> use <strong>Movies</strong> for the wytui video library — each
				channel becomes a collection and each video a standalone movie dated by its upload date. "Set
				up library" creates or fixes the library via the API and pins it to wytui's NFO metadata (no online
				matching). Run "Write NFO metadata" once to backfill existing videos.
			</div>
			<div class="jellyfin-test">
				<button
					...
					disabled={s.settingUpJellyfin || !s.settings.jellyfinUrl || !s.settings.jellyfinApiKey}
				>
...
<div class="settings-section" id="auto-delete" class:active={activeSection() === 'auto-delete'}>
	...
	<div class="form-group">
		...
		/>
		<p class="help-text">
			Automatically delete watched cache downloads after this many days. Set to 0 or leave empty to
			disable.
		</p>
	</div>
	<div class="form-group">
...
<div class="settings-section" id="rescan" class:active={activeSection() === 'rescan'}>
	...
	<p class="help-text" style="margin-bottom: var(--spacing-lg);">
		Check that downloaded files still exist on disk. Finds completed downloads whose files are
		missing.
	</p>
...
<div class="settings-section" id="notifications" class:active={activeSection() === 'notifications'}>
	...
	<div class="form-group">
		...
		/>
		<p class="help-text">
			URL of your Apprise API server for push notifications. Linked accounts with their own Apprise
			URL are notified separately.
		</p>
	</div>
...
<div class="settings-section" id="ldap" class:active={activeSection() === 'ldap'}>
	...
	{#if s.settings.ldapManagedByEnv}
		<div class="info-box managed-box" style="margin-bottom: var(--spacing-lg);">
			Managed by environment variables. LDAP settings are controlled by <code>LDAP_*</code> env vars and
...
<div class="settings-section" id="proxy-auth" class:active={activeSection() === 'proxy-auth'}>
	...
	<div class="form-group">
		...
		<p class="help-text">
			Automatically log in users based on a header set by your reverse proxy (Authelia, Authentik,
			etc.)
		</p>
	</div>
	...
	{#if s.settings.proxyAuthEnabled}
		<div class="info-box warning-box" style="margin-bottom: var(--spacing-lg);">
			Only enable this if wytui is behind a trusted reverse proxy that sets the authentication
			header. If users can reach wytui directly, they can forge the header and impersonate any user.
		</div>
		...
		<div class="form-group nested-field">
			...
			/>
			<p class="help-text">
				The HTTP header your reverse proxy sets with the authenticated username or email. Common
...
<div class="settings-section" id="oidc" class:active={activeSection() === 'oidc'}>
	...
	{#if s.settings.oidcManagedByEnv}
		<div class="info-box managed-box" style="margin-bottom: var(--spacing-lg);">
			Managed by environment variables. OIDC settings are controlled by <code>OIDC_*</code> env vars and
			cannot be changed here.
		</div>
	{/if}
	<div class="form-group">
		...
		<p class="help-text">
			Allow users to sign in through an external identity provider (Authentik, Keycloak, Google,
			etc.)
		</p>
	</div>
...
<div class="settings-section" id="auth-mode" class:active={activeSection() === 'auth-mode'}>
	...
	<div class="form-group">
		...
		<select id="authMode" bind:value={s.settings.authMode} disabled={!s.settings.oidcConfigured}>
			{#if s.settings.oidcConfigured}
				<option value="password" disabled={!s.settings.canUsePasswordOnly}
					>Password Only{!s.settings.canUsePasswordOnly ? ' (no admin has a password)' : ''}</option
				>
		...
		{#if s.settings.oidcConfigured && !s.settings.canUsePasswordOnly}
			<div class="info-box warning-box">
				Password-only mode is unavailable because no admin account has a password set. Set a
				password for an admin account to enable this option.
			</div>
		{/if}
	</div>
	...
	{#if s.settings.oidcConfigured && s.settings.authMode === 'oidc'}
		<div class="info-box warning-box">
			Password login will remain accessible at <code>/auth/signin?fallback=password</code> as a safety
			fallback in case SSO is unavailable.
		</div>
	{/if}
</div>
...
<div class="settings-section" id="config" class:active={activeSection() === 'config'}>
	...
	<div class="form-group">
		...
		<p class="help-text">
			Contains secrets (API keys, tokens, passwords) in plaintext — store the downloaded file
			securely.
		</p>
	</div>
	...
	<div class="form-group">
		...
		<p class="help-text">You'll see exactly what will change before anything is applied.</p>
	</div>
