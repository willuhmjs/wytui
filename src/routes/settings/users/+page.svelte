...
{#if s.passwordChangeUserId}
	<!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions -->
	<div class="modal-overlay" onclick={s.closePasswordChange}>
		<div
			bind:this={s.passwordModalEl}
			class="modal-content"
			role="dialog"
			aria-modal="true"
			...
			onkeydown={(e) => {
				if (e.key === 'Escape') s.closePasswordChange();
			}}
		>
			<div class="modal-header">
				<h3>Change Password</h3>
				<button class="modal-close" onclick={s.closePasswordChange}>&times;</button>
			</div>

			<div class="modal-body">
				{#if s.passwordError}
					<div class="error-message">{s.passwordError}</div>
				{/if}
				...
				>
					<div class="form-group">
						...
						<PasswordInput
							...
							bind:value={s.passwordForm.newPassword}
							placeholder="Enter new password"
							required
						/>
						{#if s.passwordForm.newPassword.length > 0}
							<div class="password-suggestions">
								<span class="suggestion" class:met={s.passwordForm.newPassword.length >= 8}
									>8+ characters</span
								>
								<span class="suggestion" class:met={/[a-z]/.test(s.passwordForm.newPassword)}
									>lowercase</span
								>
								<span class="suggestion" class:met={/[A-Z]/.test(s.passwordForm.newPassword)}
									>uppercase</span
								>
								<span class="suggestion" class:met={/[0-9]/.test(s.passwordForm.newPassword)}
									>number</span
								>
								<span class="suggestion" class:met={/[^a-zA-Z0-9]/.test(s.passwordForm.newPassword)}
									>special character</span
								>
							</div>
						{/if}
					...
					<div class="form-group">
						...
						<PasswordInput
							id="confirm-password"
							bind:value={s.passwordForm.confirmPassword}
					...
					<div class="modal-actions">
						<button type="button" class="btn btn-secondary" onclick={s.closePasswordChange}>
							Cancel
						</button>
...
<ImportSubscriptionsModal bind:open={s.showImportModal} />

<!-- Import Config Preview Modal -->
{#if s.importPreview}
	<!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions -->
	<div class="modal-overlay" onclick={s.closeImportPreview}>
		<div
			class="modal-content"
			role="dialog"
			aria-modal="true"
			...
			onclick={(e) => e.stopPropagation()}
			onkeydown={(e) => {
				if (e.key === 'Escape') s.closeImportPreview();
			}}
		>
			<div class="modal-header">
				<h3>Review Config Import</h3>
				<button class="modal-close" onclick={s.closeImportPreview}>&times;</button>
			</div>

			<div class="modal-body">
				{#if s.importError}
					<div class="error-message">{s.importError}</div>
				{/if}

				{#if s.importPreview.changes.length === 0}
				...
				{:else}
					<p class="help-text" style="margin-bottom: var(--spacing-md);">
						{s.importPreview.changes.length} setting{s.importPreview.changes.length === 1 ? '' : 's'} will
					...
					<table class="import-diff-table">
						...
						<tbody>
							{#each s.importPreview.changes as change (change.field)}
								<tr>
									<td class="import-diff-field">{change.field}</td>
									<td class="import-diff-from">{s.formatSettingValue(change.from)}</td>
									<td class="import-diff-to">{s.formatSettingValue(change.to)}</td>
								</tr>
							{/each}
						</tbody>
					</table>
				...
				{#if s.importPreview.skipped?.length}
					<p class="help-text" style="margin-top: var(--spacing-md);">
						Ignored (managed by environment variables): {s.importPreview.skipped.join(', ')}
					</p>
				{/if}

				<div class="modal-actions">
					<button type="button" class="btn btn-secondary" onclick={s.closeImportPreview}>
						Cancel
					</button>
					{#if s.importPreview.changes.length > 0}
						<button
							...
							onclick={s.applyImport}
							disabled={s.applyingImport}
						>
							{s.applyingImport ? 'Applying...' : 'Apply Changes'}
						</button>
					{/if}
