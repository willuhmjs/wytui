<script lang="ts">
	...
	function applyModePrefsWithProfile(isAudio: boolean) {
		...
		if (prefs.profileId) {
			...
			if (exists) {
				if (isAudio) {
					selectedAudioProfileId = prefs.profileId;
					selectedVideoProfileId = null;
				} else {
					selectedVideoProfileId = prefs.profileId;
		...
		},
		{
			...
			flags: [
				...
				},
				{
					...
					label: 'Audio format',
					type: 'select',
					options: ['best', 'mp3', 'aac', 'flac', 'opus', 'm4a', 'vorbis', 'wav', 'alac'],
				},
				{
					key: 'audio_quality',
					flag: '--audio-quality',
	...
	let filteredCategories = $derived.by(() => {
		...
		return FLAG_DEFINITIONS.map((cat) => {
			const categoryMatch = cat.category.toLowerCase().includes(q);
			const matchingFlags = cat.flags.filter(
				(f) =>
					categoryMatch || f.label.toLowerCase().includes(q) || f.flag.toLowerCase().includes(q),
			);
	...
	function loadProfileFlags(profile: any) {
		...
		if (profile?.customFlags?.length) {
			...
			const flagByStr = new Map(FLAG_DEFINITIONS.flatMap((c) => c.flags).map((f) => [f.flag, f]));
			let i = 0;
			while (i < cf.length) {
				const token = cf[i];
				...
				if (def) {
					...
					const cat = FLAG_DEFINITIONS.find((c) => c.flags.some((f) => f.key === def.key));
	...
	let enabledFlagCount = $derived(
		Object.entries(flags).filter(([k, f]) => f.enabled && !defaultFlagKeys.has(k)).length,
	);
	...
	let videoProfiles = $derived(profiles.filter((p: any) => p.isSystem && !p.audioOnly).slice(0, 4));
	let audioProfiles = $derived(profiles.filter((p: any) => p.isSystem && p.audioOnly).slice(0, 3));
	...
	async function handlePlaylistImport() {
		...
		const unsubComplete = onSSEEvent('playlist:import:complete', (data: any) => {
			...
			addToast(
				'success',
				`Imported ${data.created} video${data.created !== 1 ? 's' : ''} from "${data.playlistTitle}"${data.skipped ? ` (${data.skipped} skipped)` : ''}`,
			);
		});
		...
		try {
			...
			clearTimeout(playlistProgressTimer);
			playlistProgressTimer = setTimeout(() => {
				playlistProgress = null;
			}, 5000);
		}
	}
				...
				<div class="profile-group">
					<span class="profile-group-label">
						...
						{#if optionsNonDefault}
							<button type="button" class="reset-btn" onclick={resetOptions} disabled={loading}>
								Reset
							</button>
						{/if}
					</span>
					<div class="profile-buttons">
						...
						<button
							...
							disabled={loading}
							title={selectedAudioProfileId
								? 'Subtitle files will be saved separately (audio formats cannot embed subtitles)'
								: 'Subtitles will be embedded in video'}
						>
			...
			<div class="advanced-panel">
				<div class="advanced-header">
					...
					<div class="advanced-actions">
						<button
							...
							onclick={() => {
								const custom = customProfiles.find((p) => p.id === activeProfileId);
								newProfileName = custom ? custom.name : '';
						...
						<button type="button" class="btn-clear-flags" onclick={resetToDefaults}> Reset </button>
					</div>
				</div>
				...
				{#each filteredCategories as category}
					...
					<div class="flag-category">
						...
						>
							<span class="category-chevron" class:expanded={isExpanded}>&#9654;</span>
		...
		{#if playlistProgress}
			<div class="playlist-progress">
				<div class="playlist-progress-header">
					<span class="playlist-progress-title"
						>Importing: {playlistProgress.playlistTitle || 'Playlist'}</span
					>
					{#if playlistProgress.total && playlistProgress.current}
						<span class="playlist-progress-count"
							>{playlistProgress.current} / {playlistProgress.total}</span
						>
					{/if}
				</div>
					...
					<div class="playlist-progress-bar">
						<div
							...
							style="width: {Math.round(
								((playlistProgress.current || 0) / playlistProgress.total) * 100,
							)}%"
						></div>
					</div>
...
<style>
	...
	.profile-btn.active {
		...
		background-image:
			linear-gradient(var(--color-bg-tertiary), var(--color-bg-tertiary)),
	...
	.submit-row :global(.btn-accent:hover:not(:disabled)) {
		background: var(--color-accent-hover);
		box-shadow: 0 4px 12px rgba(59, 130, 246, 0.25);
		transform: translateY(-1px);
	}

	.submit-row :global(.btn-accent:active:not(:disabled)) {
